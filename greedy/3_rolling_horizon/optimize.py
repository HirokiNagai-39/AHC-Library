...
# =============================================================================
#  貪欲プレイアウト のパラメータを optuna で自動調整するスクリプト
# =============================================================================
#  【使い方】
...
#
#  【main.cpp のパラメータ対応】
#    p1 : ALPHA     評価値の距離の効かせ方 (問題ごとの評価関数の重みに相当)
#    p2 : EPS_DIST  0 除算よけ
#    p3 : HORIZON   プレイアウトで何手先まで進めるか (-1 なら最後まで)
#    p4 : NOISE     プレイアウトのノイズ幅
# =============================================================================
...
def generate_params(trial: optuna.trial.Trial) -> dict[str, str]:
    ...
    params = {
        # p1: 評価関数の重み
        "p1": str(trial.suggest_float("p1", 0.2, 3.0)),
        # p2: 0 除算よけ
        "p2": str(trial.suggest_float("p2", 0.01, 50.0, log=True)),
        # p3: プレイアウトの長さ (-1 は最後まで)
        "p3": str(trial.suggest_int("p3", -1, 40)),
        # p4: プレイアウトのノイズ幅
        "p4": str(trial.suggest_float("p4", 0.0, 0.5)),
    }
    return params
