// =============================================================================
//  [焼きなまし系 05] 円の詰め込み (重なりを無くす配置)
// =============================================================================
//  【問題】
//    1 辺 L の正方形 [0,L]^2 に、半径 r_0..r_{N-1} の円を N 個すべて置く。
//    円の中心座標 (x_i, y_i) を自由に決められる。次のペナルティの合計を最小化せよ。
//      (a) 円 i と円 j が重なっている量 max(0, r_i + r_j - dist(i,j))^2 の総和
//      (b) 円が正方形からはみ出している量 (各辺について max(0, はみ出し幅))^2 の総和
//    つまり「全部の円を重なりもはみ出しも無く詰め込めたら 0 点」。
//  【入力】
//    N L
//    r_0 ... r_{N-1}
//  【出力】
//    N 行。円 i の中心座標 "x y" (小数可)。
//  【スコア】 上記ペナルティの合計 (小さいほど良い、0 が完全配置)。
//  【入力生成方法】
//    N=60 固定。r_i は [1.0, 5.0] の一様実数。
//    L = sqrt(Σπr_i^2 / 0.80) とする (充填率 80% 相当。かなりきつく、0 点にするのは難しい)。
//    ※ 標準入力が無ければ上記の方法で内部生成する (環境変数 SEED で種を変えられる)。
//  【解法】
//   ● 考え方
//     座標が連続値なので、離散問題のように「近傍を全部試す」ことができない。
//     かわりに「1 つの円をランダムな方向へ動かす」という素朴な近傍で十分に動く。
//     重なりとはみ出しをペナルティにしてあるので、途中で重なりを許しながら
//     ぎゅうぎゅうに詰めていく動きになる (ペナルティ法)。
//     連続値の配置問題は局所解だらけで山登りだと必ず詰まるため、焼きなましが効く典型例。
//
//   ● 状態 (State)
//     各円の中心座標 x[], y[] だけ。
//
//   ● 手の作り方
//     円を 1 つ選び、現在位置から (dx, dy) だけずらす。
//     ずらす幅は確率 P_NB で 2 種類を切り替える。
//       ・大きく飛ばす (盤面全体の幅) … 別の空きへワープする動き
//       ・細かく揺らす (盤面の 5%)   … 詰めの微調整
//     盤の外に出たら端に丸める。
//
//   ● 評価値
//     Σ_{i<j} max(0, r_i + r_j - 距離)^2  (重なり量の二乗)
//     + Σ_i (はみ出し幅)^2
//     最小化で、0 になれば「重なりもはみ出しも無い配置」が完成している。
//     二乗にしてあるのは、大きな重なりを優先的に解消させるため。
//
//   ● 差分計算 / 高速化
//     動かす円と他の全円との重なりだけを見れば良いので O(N)。
//     はみ出しは O(1)。N=60 なので 1 手 60 回の距離計算で済む。
//
//   ● つまずきポイント
//     ・「動かす前のペナルティ」と「動かした後のペナルティ」を同じ関数で計算すると楽だが、
//       自分自身との重なりを数えないよう気を付ける。
//     ・移動幅を 1 種類にすると、大きすぎれば詰めができず、小さすぎれば空きへ移れない。
//       2 種類を混ぜるのが簡単で効く。
//
//   ● さらに伸ばすなら
//     ・現在の設定 (充填率 80%) では seed 0-19 のすべてで 0 点に到達している
//       (実際のペナルティは 0.001〜0.043 で、表示上 0)。改善余地は無い。
//     ・伸ばすなら充填率を 0.85 以上に上げる。そうなると
//       「移動幅を温度に連動させる」「重なっている相手の方向へ押し出す」近傍が効いてくる
//
//  【採用したライブラリ】 simulated annealing/2_hill_climbing_kick
//    実測比較 (seed 0,1,2 の合計スコア): 1_hill_climbing=1 / 2_hill_climbing_kick=0 / 3_simulated_annealing=0 / 4_multi_start_annealing=0 / 5_iterated_annealing=0
// =============================================================================
// =============================================================================
...
// #############################################################################
// # 6. ■ ここから 問題ごとに書き換える部分 ■  (円の詰め込み)
// #############################################################################
constexpr int MAXN = 200;

int   N;
float LSIZE;
static float RAD[MAXN];

struct State {
    float score;                        // ★必須: 重なり + はみ出しのペナルティ
    float x[MAXN], y[MAXN];
};

struct Move { int i; float nx, ny; };   // 円 i を (nx,ny) へ動かす
static Move g_mv;

static inline float out_pen(int i, float x, float y) {
    const float r = RAD[i];
    float p = 0.0f;
    const float a = r - x, b = x + r - LSIZE, c = r - y, d = y + r - LSIZE;
    if (a > 0) p += a * a;
    if (b > 0) p += b * b;
    if (c > 0) p += c * c;
    if (d > 0) p += d * d;
...
// 円 i を (x,y) に置いたときの「他の円との重なり + はみ出し」ペナルティ
static inline float pen_of(const State &s, int i, float x, float y) {
    float p = out_pen(i, x, y);
    for (int j = 0; j < N; j++) {
        if (j == i) continue;
        const float dx = x - s.x[j], dy = y - s.y[j];
        const float need = RAD[i] + RAD[j];
        const float d2 = dx * dx + dy * dy;
        if (d2 < need * need) { const float o = need - sqrtf(d2); p += o * o; }
    }
    return p;
}
...
float full_score(const State &s) {
    float t = 0.0f;
    for (int i = 0; i < N; i++) {
        t += out_pen(i, s.x[i], s.y[i]);
        for (int j = i + 1; j < N; j++) {
            const float dx = s.x[i] - s.x[j], dy = s.y[i] - s.y[j];
            const float need = RAD[i] + RAD[j];
            const float d2 = dx * dx + dy * dy;
            if (d2 < need * need) { const float o = need - sqrtf(d2); t += o * o; }
        }
    }
    return t;
}
...
void init_state(State &s) {
    for (int i = 0; i < N; i++) { s.x[i] = rng.nextf() * LSIZE; s.y[i] = rng.nextf() * LSIZE; }
    s.score = full_score(s);
}

inline void modify(State &s) {
    g_mv.i = (int)rng.next((uint32_t)N);
    const float w = (rng.nextf() < P_NB) ? LSIZE : LSIZE * 0.05f;   // 大きく飛ばす / 細かく揺らす
    g_mv.nx = min(LSIZE, max(0.0f, s.x[g_mv.i] + (rng.nextf() - 0.5f) * w));
    g_mv.ny = min(LSIZE, max(0.0f, s.y[g_mv.i] + (rng.nextf() - 0.5f) * w));
}

inline float calc_score(State &s) {
    const int i = g_mv.i;
    return pen_of(s, i, g_mv.nx, g_mv.ny) - pen_of(s, i, s.x[i], s.y[i]);
}

inline void apply_move(State &s) { s.x[g_mv.i] = g_mv.nx; s.y[g_mv.i] = g_mv.ny; }

void read_input() {
    if (scanf("%d %f", &N, &LSIZE) == 2 && N >= 1) {
        N = min(N, MAXN);
        for (int i = 0; i < N; i++) scanf("%f", &RAD[i]);
    } else {
        uint32_t sd = 0; pick_env("SEED", sd);
        Xor128 g; g.seed(sd * 1000003u + 205);
        N = 60;
        float area = 0.0f;
        for (int i = 0; i < N; i++) { RAD[i] = 1.0f + g.nextf() * 4.0f; area += 3.14159265f * RAD[i] * RAD[i]; }
        LSIZE = sqrtf(area / 0.80f);
    }
}

void output(const State &s) {
    string r; r.reserve((size_t)N * 20);
    char buf[64];
    for (int i = 0; i < N; i++) { snprintf(buf, sizeof(buf), "%.4f %.4f\n", (double)s.x[i], (double)s.y[i]); r += buf; }
