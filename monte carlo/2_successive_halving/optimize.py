...
# =============================================================================
#  モンテカルロ法 + 逐次絞り込み のパラメータを optuna で自動調整するスクリプト
# =============================================================================
#  【使い方】
...
#
#  【main.cpp のパラメータ対応】
#    p1 : ROLLOUT_TH  rollout の方策の閾値 (デモ用)
#    p2 : TIME_ALPHA  ターンごとの時間配分 (0=均等, 大きいほど序盤に厚く)
#    ※ MAX_SCENARIO=1 を環境変数で渡すと「シナリオ 1 本 = 貪欲法」相当になる (比較用)
#    p3 : HALVE_RATE  1 フェーズごとに候補を 1/HALVE_RATE に絞る
# =============================================================================
...
def generate_params(trial: optuna.trial.Trial) -> dict[str, str]:
    ...
    params = {
        # p1: rollout の方策のパラメータ (問題ごとの方策の重みに相当)
        "p1": str(trial.suggest_float("p1", 0.0, 5.0)),
        # p2: ターンごとの時間配分 (0=均等)
        "p2": str(trial.suggest_float("p2", 0.0, 4.0)),
        # p3: 逐次絞り込みの強さ (2 なら毎フェーズ半分に絞る)
        "p3": str(trial.suggest_int("p3", 2, 6)),
    }
    return params
