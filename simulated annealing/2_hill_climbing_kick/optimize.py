...
# =============================================================================
#  キックあり山登り のパラメータを optuna で自動調整するスクリプト
# =============================================================================
#  【使い方】
...
#
#  【main.cpp のパラメータ対応】
#    p1 : P_ORO            近傍の選び方(デモ: or-opt を選ぶ確率)
#    p2 : KICK_STRENGTH    キック 1 回で適用するランダム遷移の回数
#    p3 : STAGNATION_LIMIT 改善が無い回数がこれを超えたらキック
# =============================================================================
...
def generate_params(trial: optuna.trial.Trial) -> dict[str, str]:
    ...
    params = {
        # p1: 近傍の選び方 (デモ: or-opt を選ぶ確率)
        "p1": str(trial.suggest_float("p1", 0.0, 1.0)),
        # p2: キックの強さ (大きいほど解を壊す)
        "p2": str(trial.suggest_int("p2", 1, 30)),
        # p3: 局所解と判定するまでの非改善回数 (対数スケールで探索)
        "p3": str(trial.suggest_int("p3", 100, 1000000, log=True)),
    }
    return params
