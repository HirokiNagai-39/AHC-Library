// =============================================================================
//  山登り法 (Hill Climbing)   ---  AHC 用 高速テンプレート
// =============================================================================
//  改善する遷移だけを採用し続ける最も単純な局所探索。実装が軽く高速だが局所解で止まる。
//
//  【ファイル構成】
...
float P_ORO = 0.30f;    // p1: 近傍の選び方 (デモ: or-opt を選ぶ確率。1-p1 の確率で 2-opt)
...
// 7. 山登り法 本体
// =============================================================================
//  改善する遷移だけを採用し続ける。実装が単純で最も高速だが局所解で止まる。
//  まずこれで「差分計算が正しいか」「何回まわるか」を確認してから焼きなましへ進むと良い。
void hill_climbing(State &s, float deadline_ms) {
    int   block = ITER_PER_CHECK;       // ★ 時間計測はこの回数に 1 回だけ
    float prev  = timer.ms();
    while (true) {
        // ---- 内側ループ: 時間計測を一切しない (ここが最速であるべき) ----
        for (int it = 0; it < block; it++) {
            modify(s);                              // 遷移を 1 つ選ぶ
            const float diff = calc_score(s);       // スコア差分
            const float gain = GAIN_SIGN * diff;    // 改善量 (正なら改善)
            if (gain > ACCEPT_TH) {                 // 改善(または同スコア)なら採用
...
int main() {
    ...
    hill_climbing(state, TIME_LIMIT_MS);
