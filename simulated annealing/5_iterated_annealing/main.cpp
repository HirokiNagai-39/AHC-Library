...
// =============================================================================
//  焼きなましの繰り返し (Iterated / Reheat SA)   ---  AHC 用 高速テンプレート
// =============================================================================
//  時間を等分し、毎回「最良解から」温度を上げ直して焼きなましを繰り返す。
//
//  【ファイル構成】
...
float TEMP_START = 50.0f;   // p1: 1 ラウンド目の開始温度
float TEMP_END   = 1.0f;    // p2: 1 ラウンド目の終了温度
int   NUM_ROUNDS = 4;       // p3: 焼きなましを繰り返す回数 (時間をこの数で等分する)
float TEMP_DECAY = 0.7f;    // p4: ラウンドごとに温度に掛ける係数 (1.0 なら毎回同じ温度で焼き直す)
float P_ORO      = 0.30f;   // p5: 近傍の選び方 (デモ: or-opt を選ぶ確率)

void load_params() {
    pick_env("p1", TEMP_START);
    pick_env("p2", TEMP_END);
    pick_env("p3", NUM_ROUNDS);
    pick_env("p4", TEMP_DECAY);
    pick_env("p5", P_ORO);
}
...
constexpr bool  RESTART_FROM_BEST = true;   // 各ラウンドを最良解から始める (false なら現在解から続ける)
...
// =============================================================================
// 7-2. 焼きなましの繰り返し (リヒート / Iterated Annealing) 本体
// =============================================================================
//  時間を NUM_ROUNDS 等分し、毎回「最良解から」温度を上げ直して焼きなましをやり直す。
//  多点スタートと違って解を捨てないので、良い解の周りを何度も探索し直せる。
//  ラウンドが進むほど TEMP_DECAY で温度を下げ、だんだん局所的な改善に集中させる。
void iterated_annealing(State &s, float tl_ms) {
    static State best;
    init_state(s);                                       // ★ 初期解構築 (1 ラウンド目の出発点)
    best = s;
    const float begin = timer.ms();
    const float span  = (tl_ms - begin) / (float)max(1, NUM_ROUNDS);
    float t0 = TEMP_START, t1 = TEMP_END;
    for (int k = 0; k < NUM_ROUNDS; k++) {
        const float deadline = begin + span * (float)(k + 1);
        if constexpr (RESTART_FROM_BEST) s = best;       // 最良解から再スタート
        anneal(s, deadline, t0, t1);
        if (is_better(s.score, best.score)) best = s;
        t0 *= TEMP_DECAY; t1 *= TEMP_DECAY;              // ラウンドごとに温度を下げる
        g_round++;
...
int main() {
    ...
    iterated_annealing(state, TIME_LIMIT_MS);
