...
# =============================================================================
#  ペナルティ付き焼きなまし のパラメータを optuna で自動調整するスクリプト
# =============================================================================
#  【使い方】
...
#    p1 : TEMP_START       開始温度
#    p2 : TEMP_END         終了温度
#    p3 : PEN_START        ペナルティ係数の初期値
#    p4 : PEN_END          ペナルティ係数の最終値 (violation を残さないよう大きく)
#    p5 : P_ORO            近傍の選び方(デモ: or-opt を選ぶ確率)
# =============================================================================
...
def generate_params(trial: optuna.trial.Trial) -> dict[str, str]:
    ...
    params = {
        # p1: 開始温度 (受け入れたい素点悪化量の最大値くらい。対数スケールで探索)
        "p1": str(trial.suggest_float("p1", 1.0, 1000.0, log=True)),
        # p2: 終了温度 (無視したい素点悪化量くらい)
        "p2": str(trial.suggest_float("p2", 0.01, 10.0, log=True)),
        # p3: ペナルティ係数の初期値 (違反 1 単位と素点 1 単位の重さの比)
        "p3": str(trial.suggest_float("p3", 0.01, 100.0, log=True)),
        # p4: ペナルティ係数の最終値 (★下限を「素点で得られる最大の得」より大きく保つこと。
        #     小さくし過ぎると invalid なまま終わり、強制修復頼みになってスコアが不安定になる)
        "p4": str(trial.suggest_float("p4", 100.0, 100000.0, log=True)),
        # p5: 近傍の選び方 (デモ: or-opt を選ぶ確率)
        "p5": str(trial.suggest_float("p5", 0.0, 1.0)),
    }
    return params
...
def get_direction() -> str:
    direction = "minimize"     # ← 素点最小化 (デモの TSP はこちら)
    # direction = "maximize"   # ← 素点最大化
    return direction
